...
def classify(c):
    tl=c.get('timeline',{})
    exp,patch,disc=parse(tl.get('exploit_date')),parse(tl.get('patch_date')),parse(tl.get('public_disclosure_date'))
    grade=c.get('evidence_grade')
    if not exp: return 'exploited_vulnerability_timeline_unknown','missing exploit date'
    if exp and disc and disc < exp: return 'publicly_disclosed_before_exploitation','public disclosure predates exploitation'
