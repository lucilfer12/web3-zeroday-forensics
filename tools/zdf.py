...
def validate(verbose=False):
    ...
    for path,case in load_cases():
        ...
        if not urls:
            errors.append(f'{path}: no sources')
        for u in urls:
            try:
                q=urlparse(u)
                if q.scheme!='https' or not q.netloc:
...
def report():
    cases=load_cases()
    out=ROOT/'analysis/reports/corpus-status.md'
